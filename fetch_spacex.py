...
def download_spacex_launch_images(max_counts):
  ...
  if os.path.exists('images/spacex'):
    if len(os.listdir('images/spacex')) == 0:
      ...
      last_image_index = max([int(image_title.strip('spacex_img').split('.')[0]) for image_title in os.listdir('images/spacex')])
      image_counter = last_image_index
      spacex_images_urls = get_spacex_launch_images_urls(max_counts, last_image_index)
  ...
  for spacex_image_url in spacex_images_urls:
    ...
    status_counter += 1
    if status_counter == 1:
        print(f"Downloaded {status_counter} SpaceX image")
    elif status_counter > 1:
        print(f"Downloaded {status_counter} SpaceX images")
